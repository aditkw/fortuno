...
class Search_model extends MY_Model {
  ...
  public function getServices($offset = 0, $like = '') {
    $this->db->select('s.services_id, s.services_name, s.services_name_en, s.services_desc, s.services_desc_en, s.services_link, i.image_name, MAX(i.image_seq), c.catservices_link');
    ...
    $this->db->select('s.services_id, MAX(i.image_seq)');
    $this->db->from('services s');
    $this->db->join('catservices c', 'c.catservices_id = s.catservices_id');
    $this->db->join('image i', 's.services_id = i.parent_id');
    $this->db->where('i.image_parent_name', 'services');
  ...
  public function getPortfolio($offset = 0, $like) {
    $this->db->select('p.portofolio_id, p.portofolio_name, p.portofolio_name_en, p.portofolio_desc, p.portofolio_desc_en, p.portofolio_link, i.image_name, MAX(i.image_seq)');
    ...
    $this->db->select('p.portofolio_id, MAX(i.image_seq)');
    $this->db->from('portofolio p');
    $this->db->join('image i', 'p.portofolio_id = i.parent_id');
    $this->db->where('i.image_parent_name', 'portofolio');
    $this->db->like('p.portofolio_name', $like);
