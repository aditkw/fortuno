...
*
*/
class Partners extends Backend_Controller
{
	...
	protected $modul_file 			= 'partners';

	function index()
	{
		$array_where =  array('catinfo_id' => $this->catinfo_id, 'image_parent_name' => 'partners');

		$this->data['content'] 	= 'admin/pages/partners/view';
		$this->data['partners'] 		= $this->info_model->get_info($array_where);
	...
	public function action($param)
	{
		...
		switch ($param) {
			...
			case 'insert':
				...
				if ($this->form_validation->run() == FALSE) {
					...
					if ($size > $this->max_size) {
						...
						$partners_id = $this->info_model->insert($array_data);

						$array_image = array(
							'parent_id' => $partners_id,
							'image_parent_name' => 'partners',
			...
			$get_image= $this->image_model->get_by(array('image_parent_name' => 'partners', 'parent_id' => $id), 1, NULL, TRUE);
			// var_dump($get_image);
			// die();
			...
			case 'delete':
				...
				if ($count > 0) {
					...
					$where_image 	= array('image_parent_name' => 'partners', 'parent_id' => $id);
