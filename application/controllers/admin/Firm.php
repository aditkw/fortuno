...
*
*/
class Firm extends Backend_Controller
{
	...
	protected $modul_file 			= 'firm';

	function index()
	{
		$array_where =  array('catinfo_id' => $this->catinfo_id, 'image_parent_name' => 'firm');

		$this->data['content'] 	= 'admin/pages/firm/view';
		$this->data['firm'] 		= $this->info_model->get_info($array_where);
	...
	public function action($param)
	{
		...
		switch ($param) {
			...
			case 'ndakboleh':
				...
				if ($this->form_validation->run() == FALSE) {
					...
					if ($size > $this->max_size) {
						...
						$firm_id = $this->info_model->insert($array_data);

						$array_image = array(
							'parent_id' => $firm_id,
							'image_parent_name' => 'firm',
			...
			case 'update':
				...
				$get_image= $this->image_model->get_by(array('image_parent_name' => 'firm', 'parent_id' => $id), 1, NULL, TRUE);
				// var_dump($get_image);
				// die();
			...
			case 'ndakboleh':
				...
				if ($count > 0) {
					...
					$where_image 	= array('image_parent_name' => 'firm', 'parent_id' => $id);
