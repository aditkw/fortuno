...
*
*/
class International extends Backend_Controller
{
	protected $catinfo_id 		= '5';

	function index()
	{
		$array_where =  array('catinfo_id' => $this->catinfo_id);

		$this->data['content'] 	= 'admin/pages/international/view';
		$this->data['international'] 		= $this->info_model->get_by($array_where);;
		...
		switch ($param) {
			...
			case 'insert':
				...
				if ($this->form_validation->run() == FALSE) {
					...
					redirect(site_url('admin/international'));
				}

				else {

						$international_id = $this->info_model->insert($array_data);
			...
			case 'ndakboleh':
				...
				if ($count > 0) {
					...
					$where_image 	= array('image_parent_name' => 'international', 'parent_id' => $id);
