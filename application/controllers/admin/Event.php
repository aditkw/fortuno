...
{
	protected $catnews_id 				= 2;
	protected $max_size					= 1024 * 200;
	protected $wt 							= 70;
	protected $ht 							= 0;
	protected $image_input_name = 'image';
	protected $modul_file 			= 'event';

	function index()
	{
		$this->data['content'] 		= 'admin/pages/event/view';
		$this->data['event'] 		= $this->news_model->get_news(
			array(
				'{PRE}'.'image.image_seq' => '0',
				'{PRE}'.'image.image_parent_name' => 'event',
	...
	public function page($param)
	{
		...
		switch ($param) {
			...
			/* ---------- HALAMAN TAMBAH DATA ---------- */
			case 'add':
				$this->data['content'] 		= 'admin/pages/event/add';
				$this->load->view('admin/media', $this->data);
				break;

			/* ---------- HALAMAN EDIT DATA ---------- */
			case 'edit':
				$where_img_index					= array('parent_id' => $id, 'image_parent_name' => 'event', 'image_seq' => 0);

				$this->data['content'] 		= 'admin/pages/event/edit';
				$this->data['event'] 		= $this->news_model->get($id);
				$this->data['image_index']= $this->image_model->get_by($where_img_index, NULL, NULL, TRUE);
				$this->data['thumb_pre']	= $this->thumb_pre;

				$video = $this->data['event']->news_video;
				if (!empty($video)) {
		...
		switch ($param) {
			...
			case 'insert':
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$array_img['image_parent_name']	= 'event';
			...
			case 'update':
				...
				$where_img	= array('parent_id' => $id, 'image_parent_name' => 'event');
				...
				$this->form_validation->set_rules('title','event Title','required'.$is_unique);
				$this->form_validation->set_rules($rules);
